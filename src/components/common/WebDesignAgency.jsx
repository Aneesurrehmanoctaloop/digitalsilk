...
const WebDesignAgency = ({ head1, head2, head3, subhead3 }) => {
  return (
    <div className="bg-[#1B2C5C] py-12">
      {/* top header */}
      <div className="px-16">
        <div className="container text-white">
          <h4 className="text-center font-bold uppercase text-xl">{head1}</h4>
          <h4 className="text-center font-bold text-4xl mt-2">{head2}</h4>
          <p className="text-center font-medium text-lg mt-2  ">
            {head3}
            <p className="text-2xl font-semibold">{subhead3}</p>
          </p>
        </div>
      </div>
      {/* marquee top  */}
      <div className="mt-14 overflow-hidden">
        ...
        >
          {[...Array(2)].flatMap((_, index) => [
            <div
              key={`buddha-${index}`}
              className="border shrink-0 w-[300px] bg-[#ffffff09] flex flex-col gap-4 p-4 border-[#ffffff20] h-40"
            >
              <img src={slide1} alt="" className="w-60 mx-auto h-24" />
              <h3 className="text-center capitalize text-[#ffffff70]">
                Buddha brands
              </h3>
            </div>,
            <div
      ...
      <div className="mt-10 overflow-hidden">
        <div
          className="flex animate-marquee-two gap-8"
          style={{
