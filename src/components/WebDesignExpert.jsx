  ...
  return (
    <div className="bg-[#1B2C5F] px-6 md:px-16 py-12">
      <div className="container text-white">
        <h3 className="text-lg font-bold uppercase text-center">Our Experts Have Won Industry Awards</h3>
        <h2 className="text-4xl mt-2 font-bold uppercase text-center">Recognized Web Design Experts</h2>
          ...
          {activeTab === 'awards' && (
            <div className="grid grid-cols-2 space-y-8 md:grid-cols-4 gap-6">
              {awardImages.map((src, index) => (
                <img key={index} src={src} alt={`Award ${index}`} className="w-60 h-24 " />
              ))}
            </div>
          )}
          ...
          {activeTab === 'recognition' && (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 space-y-8">
              {recognitionImages.map((src, index) => (
                <img key={index} src={src} alt={`Recognition ${index}`} className="w-60 h-24" />
              ))}
            </div>
          )}
