  ...
  return (
    <div className="px-16 py-12">
      <div className="container">
        <h3 className="uppercase font-bold text-xl text-[#18CCFC] text-center">
          Before & After
        </h3>
        <h2 className="text-4xl font-bold text-center text-[#1B2C5F] mt-2">
          Featured Website Redesigns
        </h2>
        <p className="text-center text-lg mt-2 text-[#5F6568]">
          Our web design agency reimagines digital experiences for brands of all
          sizes and across industries.
        </p>
        <p className="text-center text-2xl font-bold mt-2 text-[#5F6568]">
      ...
      <div className="mt-14">
          ...
          {tabImages.map((img, index) => (
            ...
            >
              ...
              {activeIndex === index && (
                <IoMdArrowDropright
                  size={40}
                  className="absolute bottom-[-30px] left-[45%] rotate-90 text-[#90C404] "
                />
              )}
            </div>
          ))}
        ...
        <div className="relative mt-12 flex items-center justify-center ">
          <button
            onClick={handlePrev}
            className="absolute rotate-180 left-0 cursor-pointer text-[#18CCFC] "
          >
            <IoMdArrowDropright size={50} />
          </button>

          <div
            className={`overflow-hidden mb-10  transition-all duration-500 ease-in-out ${
              expanded ? "h-auto" : "h-[600px]"
            }`}
          >
            <img
              src={largeImages[activeIndex]}
              alt="active-tab"
              className="w-full max-w-4xl object-center shadow-xl rounded-xl"
            />
          </div>
          {!expanded && (
          ...
          <button
            onClick={handleNext}
            className="absolute right-0 cursor-pointer text-[#18CCFC] "
          >
            <IoMdArrowDropright size={50} />
          </button>
        </div>
