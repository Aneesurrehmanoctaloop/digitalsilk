  ...
  return (
    <div className='bg-[#1e3075] px-16 py-12'>
      ...
      <div className="container text-white">
        ...
        <h4 className='text-center font-bold text-4xl mt-2'>Featured Website Design Projects</h4>
          ...
          return (
            <div
              ...
              className={`w-32 h-32 cursor-pointer rounded-md flex flex-col gap-2 justify-center items-center transition-all duration-300
                ${isActive ? 'bg-white text-[#1B2C5C]' : 'bg-transparent text-white  hover:text-[#01AFE9]'}`}
            >
              <span>{data.icon}</span>
              <span className='text-lg font-medium uppercase'>{data.title}</span>
              <span><IoMdArrowDropright size={30} className={`rotate-90 ${isActive?"":"opacity-0"}`} /></span>
            </div>
          );
        })}
      </div>
